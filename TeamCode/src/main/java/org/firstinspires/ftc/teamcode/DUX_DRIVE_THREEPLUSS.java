...
//@Disabled
@TeleOp(name = "DUX_DRIVE_THREEPLUSS")
@Disabled

public class DUX_DRIVE_THREEPLUSS extends OpMode {
    ...
    public void loop() {
        ...
        if((state == 2 || state == 1 || state ==3)&& !gamepad2.right_bumper && !gamepad2.left_bumper && ready){
            state = 1;
            ready = false;
            oneshot = true;
        }
        ...
        if(state ==0 && ready && gamepad2.right_trigger>.95){
            state = 100;
            ready = false;
            oneshot = true;
        }
