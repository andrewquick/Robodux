 ...
 */
@Autonomous(name = "Red_Center_Vu", group = "Red")
@Disabled
public class Red_Center_Vu extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if(red > blue){
            ...
            robot.jewel_arm.setPosition(arm_up);
            sleep(200);
            DriveStraight((total_drive+160), 0.25);//Drive Off far enough to line up with cryptobox
        ...
        if (blue > red){
            ...
            DriveStraight(250,.1);
            robot.jewel_arm.setPosition(.145);
            DriveStraight(900, 0.55);//Knock off Jewel and drive off stone
            sleep(1000);
            robot.jewel_arm.setPosition(arm_up);
            sleep(800);
            DriveStraight((total_drive-1150), 0.3);//drive to line up with cryptobox
        ...
        if (blue == red){
            robot.jewel_arm.setPosition(arm_up);
            sleep(1000);
            DriveStraight(total_drive, .25);//Drive Off
        }
        sleep(100);
    ...
    public void DriveStraightColl(double distance, double inputpower) {
        ...
        while ((((((abs(leftposition + rightposition)) / 2) + 25) < abs(distance)) || ((((abs(leftposition + rightposition)) / 2) - 25) > abs(distance))) && (acceleration < .75 || robot.left_front_drive.getCurrentPosition()< 400)){
            ...
            acceleration = imu.getLinearAcceleration().xAccel;
...
*/
        ...
        if(robot.right_front_drive.getCurrentPosition()< 670) {
            spike = true;
        }
